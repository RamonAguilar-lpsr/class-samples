...
def saveTeam(playerList, filename):
	# open the file
    	...
    	# write to the file
	for i in playerList:
		file.write(i.name + " " + str(i.age) + " " + str(i.goals) + " " + 
str(i.jerseyNumber) + " " + i.position + "\n")
    	# close the file
    	file.close()
...
def loadTeam(filename):
	...
	while ltr != "":
		...
		list.append(Player(stringSplitter[0], stringSplitter[1], stringSplitter[2], 
stringSplitter[3], stringSplitter[4]))
...
AorB = raw_input()
# if the users choice is a B open the file that they want to open and print all the players in 
# the file
if AorB == "B" or AorB == "b":
...
# if A just print a space to save time and space because the instructions for both A and B are 
